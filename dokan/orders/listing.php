    ...
    <form id="order-filter" method="POST" class="dokan-form-inline">
        ...
        <table class="dokan-table dokan-table-striped">
            ...
            <tr>
				...
				<th><?php esc_html_e( 'Sales Rep', 'dokan-lite' ); ?></th>
                ...
                <?php if ( current_user_can( 'dokan_manage_order' ) ) { ?>
                    <th width="22%"><?php esc_html_e( 'Action', 'dokan-lite' ); ?></th>
                <?php } ?>
            </tr>
            </thead>
            <tbody>
				...
				if( in_array( 'vendor_staff', $current_user->roles ) && in_array( 'vendor_sales_rep', $current_user->roles ) ) {
					...
					<tr>
						...
						<td class="dokan-order-customer" data-title="<?php esc_attr_e( 'Customer', 'dokan-lite' ); ?>">
							...
							echo esc_html(get_user_meta( $user_id, 'licensed_business_name', true ));
							?>
						</td>
						<td> 
							<!-- Sales rep name -->
							...
							echo esc_html($_sales_rep_obj->display_name);
							?>
						</td>
						...
						<?php if ( current_user_can( 'dokan_manage_order' ) ) { ?>
							<td class="dokan-order-action" width="22%" data-title="<?php esc_attr_e( 'Action', 'dokan-lite' ); ?>">
								<?php
								...
								if( $order->get_status() == 'processing' ){
									echo '<a class="dokan-btn dokan-btn-default dokan-btn-sm tips" data-original-title="Edit Order" href="'
									...
									. '"><strong><i class="fa-solid fa-pencil"></i></strong></a>';
								}
																			   
								echo '<a class="dokan-btn dokan-btn-default dokan-btn-sm tips generate-pdf" data-original-title="Generate PDF" href="#" order-id="'.$order->get_id().'"><strong><i class="fa-solid fa-download"></i></strong></a>';
								?>
							</td>
						<?php } ?>
					</tr>
